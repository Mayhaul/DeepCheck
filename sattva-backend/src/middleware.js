...
function checkSubmission(req, res, next) {
  ...
  if (![...VALID_TYPES, "url"].includes(type))
    return res
      .status(400)
      .json({ success: false, error: "INVALID_SUBMISSION_TYPE" });
  if (["image", "video", "audio"].includes(type) && !req.file)
    return res.status(422).json({ success: false, error: "FILE_REQUIRED" });
  if (type === "text" && !req.body.claim?.trim())
    return res.status(422).json({ success: false, error: "CLAIM_REQUIRED" });
  if (
    ["article_url", "url"].includes(type) &&
    !(req.body.sourceUrl || req.body.url)?.trim()
  )
