...
export default function SpecularButton({
  ...
  useEffect(() => {
    ...
    const onPointerMove = (event) => {
      ...
      if (distance === 0) pointerAngle += Math.PI / 4;
    ...
    const update = (now) => {
      ...
      bright += (brightnessTarget - bright) * (1 - Math.exp(-delta * 8));
      lineColorValue.set(props.lineColor);
      baseColorValue.set(props.baseColor);
      program.uniforms.uAngle.value = angle;
      program.uniforms.uRadius.value = Math.min(props.radius, button.offsetHeight / 2) * dpr;
  ...
  return (
    <button
      ...
      style={{ ...sizeStyle, '--sb-radius': `${radius}px`, '--sb-tint': tint, '--sb-tint-opacity': tintOpacity, '--sb-text-color': textColor }}
    >
      <span ref={effectRef} aria-hidden="true" className="specular-button-effect" />
