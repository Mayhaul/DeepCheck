...
export const getAnalysis = async (id) =>
  (await api.get(`/analysis/${id}`)).data;
export const getReport = async (id) => (await api.get(`/report/${id}`)).data;
export const searchSources = async (query) =>
  (await api.post('/rag/search', { query })).data;
export const searchWeb = async (query) =>
  (await api.post('/web/search', { query })).data;
export const searchSourceHistory = async (publisher) =>
  (await api.post('/source/history', { publisher })).data;
